...
% data = cell(1,12);
% for j = 1:3
%     image_name = [path '\' files(10*j).name];
%     I = imread(image_name);
%     for i = 1:4
%         I_blur = imnoise(I,'gaussian',0,0.002*i);
%         data{i+4*(j-1)} = I_blur;
%         E(:,i+4*(j-1)) = double(data{i+4*(j-1)}(:));
%     end
% end
...
imshow(S_star)
% % figure,
% % subplot(1,3,1),Database(E',112,92,12,1);
% % title('input')
% % subplot(1,3,2),Database(L_star',112,92,12,1);
% % title('algorithm 1')
% % subplot(1,3,3),Database(L_check',112,92,12,1);
% % title('RPCA')
% % figure,
% % Database(S_star',112,92,12,1)
% figure,
% for j = 1:20
%     colL = L_star(:,j);
